...
import {
	...
	getListCourse,
	updateDetailClassroom, updateDetailClassroomV2,
...
const FormEditClassroom = (props: any) => {
	...
	useEffect (() => {
		...
		fetchListTeacherAndAdminSchoolBySchoolID (params);
	...
	const formatedListTeacher =
		      listTeacherAndAdminSchool &&
		      listTeacherAndAdminSchool.map ((teacher: any) => ( {
			      value : teacher.id,
			      label : teacher.name,
		      } ));
	...
	const onSubmit = (data: any) => {
		...
		// );
		updateDetailClassroomV2 (convertedData)
			.then ((res: any) => {
				if (res.code === 200) {
